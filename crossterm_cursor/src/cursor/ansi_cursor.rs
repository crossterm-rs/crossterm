...
impl ITerminalCursor for AnsiCursor {
    fn goto(&self, x: u16, y: u16, stdout: &Option<&Arc<TerminalOutput>>) -> Result<()> {
        write_cout!(stdout, &format!(csi!("{};{}H"), y + 1, x + 1))?;
        Ok(())
    }
