...
use crate::event::{
    filter::EventFilter,
    internal::{lock_event_reader, poll, read, InternalEvent},
    sys::Waker,
    Event,
};
    ...
    fn default() -> Self {
        ...
        thread::spawn(move || {
            while let Ok(task) = receiver.recv() {
                loop {
                    if let Ok(true) = poll(None, &EventFilter) {
                        break;
                    }
        ...
        EventStream {
            poll_internal_waker: lock_event_reader().waker(),
...
impl Stream for EventStream {
    ...
    fn poll_next(self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Option<Self::Item>> {
        let result = match poll(Some(Duration::from_secs(0)), &EventFilter) {
            Ok(true) => match read(&EventFilter) {
