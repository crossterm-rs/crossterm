...
use crate::{
    event::{
        ...
        internal::{poll, read, InternalEvent},
    },
...
fn read_position_raw() -> io::Result<(u16, u16)> {
    ...
    loop {
        match poll(Some(Duration::from_millis(2000)), &CursorPositionFilter) {
            Ok(true) => {
                if let Ok(InternalEvent::CursorPosition(x, y)) = read(&CursorPositionFilter) {
                    return Ok((x, y));
                }
            }
            Ok(false) => {
