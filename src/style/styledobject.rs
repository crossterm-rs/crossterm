...
mod tests {
    ...
    fn test_set_fg_bg_add_attr() {
        ...
        let mut styled_object = content_style.apply("test");
        ...
        assert_eq!(styled_object.content_style.fg_color, Some(Color::Green));
        assert_eq!(styled_object.content_style.bg_color, Some(Color::Magenta));
        assert_eq!(styled_object.content_style.attrs.len(), 2);
        assert_eq!(styled_object.content_style.attrs[0], Attribute::Reset);
        assert_eq!(styled_object.content_style.attrs[1], Attribute::NoItalic);
    }
}
